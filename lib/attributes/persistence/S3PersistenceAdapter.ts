...
/**
 * Implementation of {@link PersistenceAdapter} using AWS SDK v3 S3 Client
 */
export class S3PersistenceAdapter implements PersistenceAdapter {
    ...
    public async getAttributes(requestEnvelope: RequestEnvelope): Promise<{ [key: string]: string }> {
        ...
        const getParams: GetObjectCommand = new GetObjectCommand({
        ...
        try {
            data = await this.s3Client.send(getParams);
        } catch (err) {
            ...
            throw createAskSdkError(
                ...
                `Could not read item (${objectId}) from bucket (${getParams.input.Bucket}): ${err.message}`,
            );
        }
    ...
    public async saveAttributes(requestEnvelope: RequestEnvelope, attributes: { [key: string]: string }): Promise<void> {
        ...
        const putParams: PutObjectCommand = new PutObjectCommand({
            Bucket: this.bucketName,
            Key: objectId,
        ...
        try {
            await this.s3Client.send(putParams);
        } catch (err) {
            throw createAskSdkError(
                this.constructor.name,
                `Could not save item (${objectId}) to bucket (${putParams.input.Bucket}): ${err.message}`,
            );
        }
    }
    ...
    public async deleteAttributes(requestEnvelope: RequestEnvelope): Promise<void> {
        ...
        const deleteParams: DeleteObjectCommand = new DeleteObjectCommand({
            Bucket: this.bucketName,
            Key: objectId,
        });
        ...
        try {
            ...
            throw createAskSdkError(
                this.constructor.name,
                `Could not delete item (${objectId}) from bucket (${deleteParams.input.Bucket}): ${err.message}`,
            );
        }
    }
}
